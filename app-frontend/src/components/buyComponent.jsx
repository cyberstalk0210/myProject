...
function BuyProducts() {
  ...
  const handleQuantityChange = (id, quantity) => {
    setSelectedProducts((prev) => ({
      ...prev,
      [id]: Math.max(0, Number(quantity) || 0), // Ensure non-negative quantity
    }));
  };
  ...
  // Handle checkout (to be implemented based on your backend)
  ...
  try {
    ...
    // Send API request for each selected product
    const response = await Promise.all(
      purchaseData.map((data) =>
        axios.post('http://localhost:8085/api/create/order', data, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('AccessToken')}`,
    ...
    // Optionally reset selected products after successful checkout
    setSelectedProducts({});
  } catch (error) {
